    ...
    switch (data) {
        case "select":
            client.db.query(`SELECT * FROM dutylogs WHERE id = ?;`, [Number(interaction.values[0])], function (err, res) {
                client.db.query(`SELECT * FROM players WHERE steam = ?;`, [res[0].steam], function (err, playerRes) {
                    let discord = playerRes[0]?.discord || "Not linked";
                    let log = res[0];
                    ...
                    let row = [
                        new Row()
                            .addComponent(
                                new Button()
                                    .setCustomId(`runreport-goback-${interaction.member.id}`)
                                    .setLabel("Back")
                                    .setStyle(3)
                                    .setEmoji('🔙')
                            )
